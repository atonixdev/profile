...
// ATONIXCORP — Financial Dashboard Shell
const NAV_ITEMS = [
  { path: '/financial-dashboard',                 exact: true,  label: 'Master Finance',           code: 'MFA' },
  { path: '/financial-dashboard/revenue',         exact: false, label: 'Product Revenue',         code: 'REV' },
  { path: '/financial-dashboard/cost',            exact: false, label: 'Product Cost',            code: 'CST' },
  { path: '/financial-dashboard/budget',          exact: false, label: 'Budget & Forecast',       code: 'BDG' },
  { path: '/financial-dashboard/billing',         exact: false, label: 'Billing & Payments',      code: 'BLP' },
  { path: '/financial-dashboard/department',      exact: false, label: 'Department Finance',      code: 'DPT' },
  { path: '/financial-dashboard/vendor',          exact: false, label: 'Vendor & Procurement',    code: 'VND' },
  { path: '/financial-dashboard/compliance',      exact: false, label: 'Compliance & Audit',      code: 'CMP' },
];

const ACCENT  = '#A81D37';
const PRIMARY_ACCENT = '#1F2937';
const SIDEBAR_BG     = '#0f1623';
const SIDEBAR_BORDER = 'rgba(255,255,255,0.10)';
const HEADER_BG      = '#FFFFFF';

const FinancialDashboardLayout = () => {
  ...
  const displayName = [user?.user?.first_name, user?.user?.last_name]
    .filter(Boolean).join(' ') || user?.username || 'Executive';
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <header style={{
          ...
          >
            <div style={{ width: 18, height: 2, background: PRIMARY_ACCENT, marginBottom: 4 }} />
            <div style={{ width: 18, height: 2, background: PRIMARY_ACCENT, marginBottom: 4 }} />
            <div style={{ width: 18, height: 2, background: PRIMARY_ACCENT }} />
          </button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {currentItem && (
              <>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.10em', color: PRIMARY_ACCENT, textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>
