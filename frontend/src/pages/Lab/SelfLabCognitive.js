    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Run memory, focus, and cognition tests.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="text-sm font-semibold text-gray-700">Test</div>
          <select
            value={mode}
            onChange={(e) => {
              setError('');
          ...
          <div className="text-xs text-gray-500 sm:ml-auto">Results are saved to your Self Lab history.</div>
        </div>

        {mode === 'memory' && (
          <div className="space-y-3">
            <div className="text-sm text-gray-700">Memorize the digits, then type them back.</div>
            {memoryPhase === 'idle' && (
              <button
                type="button"
                onClick={startMemory}
            ...
            {memoryPhase === 'show' && (
              <div className="p-4 rounded-lg border border-gray-200 bg-gray-50">
                <div className="text-xs text-gray-500 mb-2">Remember:</div>
                <div className="text-3xl font-mono font-bold text-gray-900 tracking-widest">{memoryDigits}</div>
              </div>
            )}
            {memoryPhase === 'recall' && (
              <div className="space-y-3">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-gray-300 text-gray-900 outline-none font-mono"
            ...
            {memoryPhase === 'done' && (
              <button
                type="button"
                onClick={startMemory}
                className="px-5 py-2.5 border border-gray-200 rounded-lg font-semibold hover:bg-gray-50"
                disabled={saving}
              >
                Run Again
              </button>
        ...
        {mode === 'focus' && (
          <div className="space-y-3">
            <div className="text-sm text-gray-700">Count how many times the target letter appears.</div>
            {focusPhase === 'idle' && (
              <button
                type="button"
                onClick={startFocus}
            ...
            {focusPhase !== 'idle' && (
              <div className="p-4 rounded-lg border border-gray-200 bg-gray-50">
                ...
                <div className="text-lg font-bold text-gray-900">{focusTarget}</div>
                <div className="text-xs text-gray-500 mt-2">Stream</div>
                <div className="mt-1 font-mono text-sm text-gray-800 break-all">{focusStream}</div>
              </div>
            )}
            {focusPhase === 'running' && (
              <div className="space-y-3">
        ...
        {mode === 'reaction' && (
          <div className="space-y-3">
            <div className="text-sm text-gray-700">Click when you see GO.</div>
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-lg font-semibold border ${reactionPhase === 'go' ? 'bg-green-600 text-white border-green-700' : 'border-gray-200 hover:bg-gray-50'}`}
                disabled={saving || reactionPhase === 'idle'}
              >
                {reactionPhase === 'waiting' ? 'WAIT…' : reactionPhase === 'go' ? 'GO!' : reactionPhase === 'done' ? 'Done' : 'Click'}
              </button>
              {reactionMs != null && (
                <div className="text-sm text-gray-700">Reaction: <span className="font-bold">{reactionMs} ms</span></div>
              )}
            </div>
            {reactionPhase === 'done' && (
              <button
        ...
        {saving && <div className="text-xs text-gray-500">Saving result…</div>}
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {loading ? (
          ...
          <div className="text-gray-600">No cognitive experiments yet.</div>
        ) : (
              ...
              <tbody>
                {selectedItems.map((r) => (
                  <tr key={r.id} className="border-b last:border-b-0">
                    ...
                    <td className="py-2 text-gray-600">{r.test_type}</td>
                    <td className="py-2 text-gray-600">{r.score ?? '—'}</td>
                    <td className="py-2 text-gray-600">{r.duration_ms != null ? `${r.duration_ms} ms` : '—'}</td>
