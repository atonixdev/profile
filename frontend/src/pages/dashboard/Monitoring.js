  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
        {METRICS.map((m) => (
          <div key={m.label} style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', padding: '22px 20px' }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#9CA3AF', fontFamily: 'var(--font-mono)', marginBottom: 10 }}>
              {m.label}
            </div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ fontSize: 32, fontWeight: 900, color: '#9CA3AF', lineHeight: 1 }}>{m.value}</span>
              <span style={{ fontSize: 12, color: '#9CA3AF', fontFamily: 'var(--font-mono)' }}>{m.unit}</span>
            </div>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderTop: 'none', padding: '64px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: '#9CA3AF', lineHeight: 1.65, maxWidth: 440, margin: '0 auto' }}>
