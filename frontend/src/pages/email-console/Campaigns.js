...
export default function EmailCampaigns() {
  ...
  const fetchCampaigns = useCallback(async (filter = '') => {
    ...
    try {
      ...
      if (!ct.includes('application/json')) throw new Error(`Backend not reachable (got ${res.status})`);
  ...
  const fetchMeta = useCallback(async () => {
    try {
      ...
      const [tRes, sRes] = await Promise.all([
        fetch('/api/admin/templates/?page_size=200', { credentials: 'include', headers: J }),
        fetch('/api/admin/senders/', { credentials: 'include', headers: J }),
      ]);
      if (tRes.ok) { const d = await tRes.json(); setTemplates(d.results || d); }
      if (sRes.ok) { const d = await sRes.json(); setSenders(d.results || d); }
    } catch (_) {}
  }, []);
  ...
  const handleSend = useCallback(async (campaignId) => {
    if (!window.confirm('Send this campaign immediately to all recipients?')) return;
    ...
    <div style={{ padding: 'clamp(16px, 4vw, 32px) clamp(16px, 4vw, 36px)', color: '#1F2937', minHeight: '100%' }}>
      ...
      <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, color: '#111827' }}>Email Campaigns</h1>
          <p style={{ fontSize: 13, color: '#4B5563', margin: '6px 0 0' }}>
            Create, schedule, and dispatch email campaigns.
          </p>
        </div>
        <button
          onClick={() => { setShowNew((v) => !v); setSaveMsg(null); }}
          style={{
            padding: '9px 20px', background: showNew ? 'transparent' : A,
      ...
      {showNew && (
        <div style={{ ...CARD, marginBottom: 24, border: '1px solid rgba(212,175,55,0.35)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: 14 }}>
            ...
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: '#4B5563', display: 'block', marginBottom: 5, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Sender</label>
              <select
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center' }}>
        <select
          ...
          style={{ padding: '7px 12px', background: '#FFFFFF', border: BD, color: '#1F2937', fontSize: 12, fontFamily: 'var(--font-mono)', outline: 'none', cursor: 'pointer' }}
        >
      ...
      {loading ? (
        ...
        <div style={{ background: '#FFFFFF', border: BD, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                      ...
                      <div style={{ display: 'flex', gap: 5, flexWrap: 'nowrap' }}>
                        {(c.status === 'draft' || c.status === 'scheduled') && (
                          <button
                            ...
                            style={{ fontSize: 9, padding: '4px 10px', cursor: 'pointer', background: 'transparent', border: `1px solid ${A}`, color: A, fontFamily: 'var(--font-mono)', fontWeight: 700, whiteSpace: 'nowrap', opacity: sendingId === c.id ? 0.6 : 1 }}
                          >
                            {sendingId === c.id ? '…' : 'Send'}
                          </button>
                        )}
                        <button
                          onClick={() => openLogs(c)}
                          style={{ fontSize: 9, padding: '4px 10px', cursor: 'pointer', background: 'transparent', border: BD, color: '#4B5563', fontFamily: 'var(--font-mono)', fontWeight: 700, whiteSpace: 'nowrap' }}
                        >
                          Logs
                        </button>
                        <button
                          onClick={() => handleDelete(c.id)}
                          style={{ fontSize: 9, padding: '4px 10px', cursor: 'pointer', background: 'transparent', border: '1px solid rgba(239,68,68,0.3)', color: '#EF4444', fontFamily: 'var(--font-mono)', fontWeight: 700, whiteSpace: 'nowrap' }}
                        >
                          Del
                        </button>
                      </div>
